...
interface SimulationWithCompanies {
  id: string;
  companies: Array<{
    id: string;
    name: string;
...
}> = React.memo(({ label, companies, getValue, format }) => (
    ...
    {companies.map((company) => (
      <div key={company.id} className="text-white text-right">
        {format
          ...
          : `$${getValue(company).toLocaleString()}`}
      </div>
    ))}
  </div>
));
...
const ComparePage: React.FC = () => {
  ...
  // Debug re-renders
  console.log("ComparePage re-rendered at:", new Date().toISOString());
    ...
    const loadCompanies = async () => {
      try {
        ...
        const currentSim = simulations.find(
          (s: SimulationWithCompanies) => s.id === simulationId
        );
  ...
  // Optimized callback for company selection
  ...
  }, [simulationId, selectedCompanyIds]);

  const handleRefresh = useCallback(() => {
    if (selectedCompanyIds.length >= 2) {
      handleCompareClick();
    }
  }, [selectedCompanyIds.length, handleCompareClick]);
