...
export async function getCompanyDashboardData(companyId: string) {
  ...
  const latestHrDecision = hrMetricsRaw.length > 0 ? hrMetricsRaw[hrMetricsRaw.length - 1] : null;
  const latestRdDecision = rd_decision.length > 0 ? rd_decision[rd_decision.length - 1] : null;
  const latestProductionDecision = production_decision.length > 0 ? production_decision[production_decision.length - 1] : null;
  const latestMarketingDecision = marketing_decision.length > 0 ? marketing_decision[marketing_decision.length - 1] : null;
  ...
  return {
    ...
    finance_decision: financialHistory.length > 0 ? financialHistory[financialHistory.length - 1] : null,
