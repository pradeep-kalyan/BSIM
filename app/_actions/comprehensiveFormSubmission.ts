...
import { validateFormData, calculateTotalBudget } from "./formValidation";
...
export async function comprehensiveFormSubmission(
  ...
  try {
    ...
    // Step 1: Verify company and check funds in a separate transaction
    console.log("Step 1: Verifying company and funds...");
    const companyVerification = await prisma.$transaction(
      async (tx) => {
        ...
        const totalBudgetRequired = calculateTotalBudget(formData);

        if (company.cash_balance < totalBudgetRequired) {
          throw new Error(
            `Insufficient funds. Required: ${totalBudgetRequired}, Available: ${company.cash_balance}`
          );
        }

        return { company, totalBudgetRequired };
      },
      {
    ...
    const { company, totalBudgetRequired } = companyVerification;
    ...
    // Step 2: Process each business area in separate transactions to avoid timeouts

    // 2.1. Submit HR Decision
    console.log("Step 2.1: Processing HR decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
          ...
          if (existingHRDecision) {
            ...
            submissionResults.hr = { success: true, id: updatedHRDecision.id };
          } else {
    ...
    console.log("Step 2.2: Processing Marketing decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
          ...
          const conversion_rate =
            Math.round((formData.marketing.budget / 10000) * 100) / 100;
    ...
    console.log("Step 2.3: Processing R&D decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
          ...
          if (existingRDDecision) {
            ...
            submissionResults.rd = { success: true, id: updatedRDDecision.id };
          } else {
      ...
      submissionResults.rd = {
        success: false,
        error: error instanceof Error ? error.message : "R&D submission failed",
      };
      console.error("R&D submission error:", error);
    }

    // 2.4. Submit Production Decision
    console.log("Step 2.4: Processing Production decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
    ...
    console.log("Step 2.5: Processing Finance decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
      ...
      submissionResults.finance = {
        success: false,
        error:
          error instanceof Error ? error.message : "Finance submission failed",
      };
      console.error("Finance submission error:", error);
    }

    // 2.6. Handle Product (Create or Update)
    console.log("Step 2.6: Processing Product decisions...");
    try {
        ...
        await prisma.$transaction(
          async (tx) => {
            ...
            if (existingProduct) {
              ...
              submissionResults.product = { success: true, id: newProduct.id };
            }
          },
          {
    ...
    // Step 2.7: Process Sales decisions as Product Performance
    console.log("Step 2.7: Processing Sales decisions...");
    try {
      await prisma.$transaction(
        async (tx) => {
    ...
    try {
      await prisma.$transaction(
        async (tx) => {
          // Deduct total budget from company cash balance
          await tx.company.update({
            where: { id: companyId },
            data: {
              cash_balance: { decrement: totalBudgetRequired },
            },
          });
